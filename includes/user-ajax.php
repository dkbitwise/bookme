        ...
        <div class="bookme-container">
            ...
            <div class="bookme-row">
                ...
                <div class="packagedetails">
                    ...
                    <div class="studentdetailsnew"><strong>Date: </strong><?php echo date( "M d, Y", strtotime( $dates ) ); ?> (<?php echo date( "l", strtotime( $dates ) ); ?>)</div>
                </div>
                            ...
                            <div class="<?php echo ( bookme_get_settings( 'pmt_local', 'enabled' ) != 'disabled' || bookme_get_settings( 'pmt_paypal', 'disabled' ) != 'disabled' || bookme_get_settings( 'pmt_stripe', 'disabled' ) != 'disabled' || ( class_exists( 'WooCommerce', false ) && ( bookme_get_settings( 'enable_woocommerce', 0 ) != 0 ) ) ) ? 'col-md-6 bookme-col-sm-12 bookme-col-xs-12' : 'bookme-col-xs-12'; ?>" style="width:100%">
                                ...
                                <div class="bookme-calender">
                                    <div class="bookme-aval-time">
                                        ...
                                        <p style="text-align: center!important; font-weight: bold; font-size: 19px;"><?php echo date( "D", strtotime( $holidaycheck[0] ) ) ?> <?php echo date_i18n( get_option( 'time_format' ), strtotime( $appointstart ) ) . " to " . date_i18n( get_option( 'time_format' ), strtotime( $appointend ) ); ?>
                                            (<?php echo $_SESSION['tzone']; ?>)</p>
                                        ...
                                        <ul class="availabledates">
											...
											foreach ( $holidaycheck as $fulldates ) {
												...
												<?php } else {
                                                    ...
                                                    <li class="enroll">
                                                        <span class="enrolldates"><i style="padding-right:5px;" class="fa fa-calendar" aria-hidden="true"></i><?php echo $fulldates; ?></span><span class="enrolltime"><i style="padding-right:5px;" class="fa fa-clock-o" aria-hidden="true"></i><?php echo $booking_day ?> <?php echo date_i18n( get_option( 'time_format' ), strtotime( $appointstart ) ) . " to " . date_i18n( get_option( 'time_format' ), strtotime( $appointend ) ); ?></span>
                                                    </li>
                                        ...
                                        <div class="notavailabletext">
                                            <div class="leaveindicate"></div>
                                            <span style="padding-left: 5px;font-weight: bold;">Tutor not available</span></div>
										<?php if ( count( $commonholidayreason ) ) { ?>
                                            <div class="notavailabletext">
                                                <div class="holidayindicate"></div>
                                                <span style="padding-left: 5px;font-weight: bold;">Holiday(s): <?php if ( count( $commonholidayreason ) > 0 ) {
														foreach ( $commonholidayreason as $holidaycomm ) {
															echo $holidaycomm . ' ';
														}
													} ?></span></div>
										<?php }
					...
					if ( $person <= $avl ) {
						if ( $wpdb->insert( $table_customers, array(
							...
							if ( $wpdb->num_rows > 0 ) {
								...
								$wpdb->insert( $table_current_booking, array(
									'cat_id'   => $category,
									'ser_id'   => $service,
									'emp_id'   => $employee,
									'date'     => $dates,
							...
							foreach ( $custom_text as $custom ) {
								$wpdb->insert( $table_current_booking_fields, array(
									'booking_id' => $customer_booking_id,
									'key_field'  => base64_decode( $custom['name'] ),
									'field_val'  => $custom['value'],
									'status'     => 'valid'
								) );

								if ( $custom['value'] != '' ) {
									$custom_fields_text .= sprintf( "%s: %s\n", base64_decode( $custom['name'] ), $custom['value'] );

									$custom_fields_html .= sprintf( '<tr valign=top><td>%s:&nbsp;</td><td>%s</td></tr>', base64_decode( $custom['name'] ), $custom['value'] );
								}
							}
							...
							foreach ( $custom_checkbox as $custom ) {
								...
								$wpdb->insert( $table_current_booking_fields, array(
									'booking_id' => $customer_booking_id,
									'key_field'  => base64_decode( $custom['name'] ),
									'field_val'  => implode( ',', $custom['value'] ),
									'status'     => 'valid'
								) );
								...
								if ( $custom['value'] != '' ) {
									...
									$custom_fields_html .= sprintf( '<tr valign=top><td>%s:&nbsp;</td><td>%s</td></tr>', base64_decode( $custom['name'] ), implode( ',', $custom['value'] ) );
								}
							}
